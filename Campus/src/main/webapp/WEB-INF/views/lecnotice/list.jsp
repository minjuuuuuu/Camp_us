...
<script>
	...
	function goToDetail(id) {
		location.href = '${pageContext.request.contextPath}/lecnotice/detail?lecNoticeId='
				+ id;
	}
</script>
</head>
<body>
	...
	<div class="wrapper">
		<div class="content-wrapper">
			...
			<form method="get"
				action="${pageContext.request.contextPath}/lecnotice/list">
				<div class="top-bar">
			...
			<table>
				...
				<tbody>
					...
					<c:forEach var="notice" items="${lecNoticeList}" varStatus="status">
						<tr onclick="goToDetail('${notice.lecNoticeId}')"
							style="cursor: pointer;">
							<td>${pageMaker.totalCount - ((page - 1) * pageMaker.perPageNum + status.index)}</td>

							<td style="text-align: left;"><span class="title-text">${notice.lecNoticeName}</span>
								<c:set var="hasAttach"
									value="${
								...
								<c:if test="${hasAttach}">
									<img
										src="${pageContext.request.contextPath}/resources/images/clip.png"
										alt="첨부" class="clip-icon">
								</c:if></td>

							<td>${notice.profesId}</td>
							<td><fmt:formatDate value="${notice.lecNoticeDate}"
